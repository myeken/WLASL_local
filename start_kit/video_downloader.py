...
def save_video(data, saveto):
    if data is None:
        logging.error(f"No data to save for {saveto}")
        return
...
def download_nonyt_videos(indexfile, saveto='raw_videos'):
    # Define the signs you want to download
    target_signs = {"book", "hello", "thank you"}  # Reverted to original signs

    content = json.load(open(indexfile))

    # Filter the dataset to include only the selected signs
    filtered_content = [entry for entry in content if entry["gloss"] in target_signs]
    ...
    for entry in filtered_content:
        ...
        # Create a folder for each gloss (sign)
        gloss_folder = os.path.join(saveto, gloss)
        ...
        for inst in instances:
            ...
            try:
                # Save the video in the gloss-specific folder
                download_method(video_url, gloss_folder, video_id)
            except Exception as e:
                logging.error('Unsuccessful downloading - video {}: {}'.format(video_id, e))
...
def download_yt_videos(indexfile, saveto='raw_videos'):
    ...
    for entry in filtered_content:
        ...
        for inst in instances:
            ...
            if os.path.exists(os.path.join(gloss_folder, video_url[-11:] + '.mp4')) or os.path.exists(os.path.join(gloss_folder, video_url[-11:] + '.mkv')):
                logging.info('YouTube videos {} already exists.'.format(video_url))
                continue
            else:
                cmd = f"{youtube_downloader} \"{{}}\" -o \"{{}}%(id)s.%(ext)s\""
                cmd = cmd.format(video_url, gloss_folder + os.path.sep)
